import type {  outputResultOfScrapingRow, outputRobotsCsvRow } from "@config/conf-file";
import type { Browser } from "puppeteer";
import type { HandlerCallback } from 'stream-transform';
import { outputPickedTargetCsvDirPath, outputResultOfScrapingDirPath } from "@config/conf-file";
...
export async function createPdf(): Promise<boolean> {
    const tgtCsvPath = globFirstFilePathOfTgtExt(outputPickedTargetCsvDirPath, "csv");
    if (!tgtCsvPath) {
    ...
    try {
        ...
        const transformer = transform<outputRobotsCsvRow,unknown>( async (record,callback:HandlerCallback<outputResultOfScrapingRow>) => {
                if (browser === null) throw new TypeError("Browser is unintentionally null.");
        ...
        const csvWriter = getWSIntoCsv(outputResultOfScrapingDirPath, "picked-target-output.csv");
