...
const mockedPageGoto = (
    jest.fn() as jest.MockedFunction<Page["goto"]>
).mockImplementation(async () => {
    return new HTTPResponse();
});
const mockedPageTitle = (
    jest.fn() as jest.MockedFunction<Page["title"]>
).mockResolvedValue("SOME-PAGE-TITLE");
const mockedResStatus = jest.fn() as jest.MockedFunction<
    HTTPResponse["status"]
>;
...
jest.mock("puppeteer", () => {
    return {
        ...
        Browser: function (): Partial<puppeteer.Browser> {
            return {
                newPage: (
                    jest.fn() as jest.MockedFunction<Browser["newPage"]>
                ).mockImplementation(async () => {
                    return new Page();
                }),
            };
...
const GENERATED_KEY = "APPLELEMON";
jest.mock("@util/crypto/generate-random-key", () => {
    return {
        __esModule: true,
        generateRandomKey: jest.fn().mockImplementation(() => GENERATED_KEY),
    };
});
...
describe("handle various cases of accessing a webpage", () => {
    it("for a page that can be saved as pdf", async () => {
        ...
        expect(spyedJoin.mock.calls[0][1]).toBe(
            `SOME-PAGE-TITLE_${GENERATED_KEY}.pdf`
        );
...
it('should sanitize the page"s title', async () => {
    ...
    expect(loggerMock.error).not.toHaveBeenCalled();
    expect(spyedJoin.mock.calls[0][1]).toBe(
        `sanitization-required-title_${GENERATED_KEY}.pdf`
    );
    expect(mockedPdfCreationFunc).toHaveBeenCalled();
